...
SYSTEM_PROMPT = """You are an expert curriculum designer and content creator for English listening practice (bilingual English-Vietnamese).
...
- topic (string, a highly specific situation generated from the broad theme, e.g., "Morning Coffee Routine")
- title_en (string, uppercase style, short, catchy)
- title_vi (string, short, natural Vietnamese translation of the title)
- text_en (string, 45-70 English words, clear, one topic, spoken conversational tone)
- text_vi (string, natural Vietnamese translation of text_en)
...
- Keep text_en short enough for a ~25-35s narration at normal speed.
- Avoid overly formal, textbook-style sentences.
- text_vi must perfectly capture the nuanced, casual tone of the English text.
"""
...
2) text_en must be 45-70 English words. It should be a 1-person monologue (e.g., someone talking about their experience).
...
def validate_payload(payload: dict[str, Any], target_duration: int) -> list[str]:
    ...
    for field in ("topic", "title_en", "title_vi", "text_en", "text_vi", "difficulty"):
    ...
    if isinstance(text_en, str):
        ...
        if words < 45 or words > 70:
            errors.append(f"text_en should be 45-70 English words (current: {words})")
...
def resolve_project_output_path(
    ...
    topic_dir = output_root / topic_to_slug(topic)
    topic_dir.mkdir(parents=True, exist_ok=True)

    candidate = topic_dir / project_id
    ...
    counter = 1
    while True:
        alt = topic_dir / f"{project_id}_{counter}"
        if not alt.exists():
            return alt
        counter += 1
...
def main() -> None:
    ...
    output_root = args.output_root or (project_dir / "output" / "short_video")
