...
TMP_AUDIO_DIR = os.path.join(OUTPUT_DIR, "tmp_vibevoice")
...
# Speaker map aligned with make_video_with_image capability.
SPEAKER_VOICE_CANDIDATES: Dict[str, List[str]] = {
    "Alex": ["en-Davis_man.pt", "en-Frank_man.pt", "en-Mike_man.pt", "en-Carter_man.pt"],
    "Sarah": ["en-Emma_woman.pt", "en-Grace_woman.pt"],
    "Michael": ["en-Mike_man.pt", "en-Frank_man.pt", "en-Davis_man.pt"],
    "Nicole": ["en-Grace_woman.pt", "en-Emma_woman.pt"],
    "Adam": ["en-Frank_man.pt", "en-Carter_man.pt", "en-Mike_man.pt"],
    "Sky": ["en-Grace_woman.pt", "en-Emma_woman.pt"],
    # Backward compatibility with old script naming
    "Davis": ["en-Davis_man.pt", "en-Frank_man.pt"],
    "Emma": ["en-Emma_woman.pt", "en-Grace_woman.pt"],
}
DEFAULT_VOICE_CANDIDATES = ["en-Davis_man.pt", "en-Emma_woman.pt", "en-Frank_man.pt"]
CFG_SCALE = 1.5
TESTING_MODE = False
...
def get_voice_for_speaker(speaker: str) -> str:
    if speaker in SPEAKER_VOICE_CANDIDATES:
        return resolve_voice_path(SPEAKER_VOICE_CANDIDATES[speaker])

    for key, candidates in SPEAKER_VOICE_CANDIDATES.items():
        if key.lower() in speaker.lower():
            return resolve_voice_path(candidates)

    return resolve_voice_path(DEFAULT_VOICE_CANDIDATES)
...
def should_skip_audio(item: Dict, idx: int) -> bool:
    ...
    # Mute heading behavior aligned with make_video_with_image.
    if item_type == "heading" and (idx == 1 or not text_tts.strip() or not speaker.strip()):
        return True
    return False


def process_topic_tts(topic: str, model, processor, device: str):
    ...
    print(f"Found {total_turns} turns to process.")
    ...
    for idx, item in enumerate(items, start=1):
        ...
        try:
            voice_path = get_voice_for_speaker(speaker or "Alex")
        ...
        speaker_safe = "".join(ch for ch in speaker_name if ch.isalnum() or ch in ("_", "-")) or "unknown"
        ...
        base_filename = f"{speaker_safe.lower()}_{idx}"
        ...
        print(
            ...
            f"for {speaker_name} ({duration_ms / 1000.0:.1f}s)"
        )
